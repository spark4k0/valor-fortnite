...
namespace vdm
{
	...
	public:
		...
		{
			T buffer;
			if (!rkm(reinterpret_cast<void*>(&buffer), reinterpret_cast<void*>(addr), sizeof(T)))
			{
				return {};  
			}
			return buffer;
		...
		template <class T>
		__forceinline bool wkm(std::uintptr_t addr, T& value)
		{
			return wkm(reinterpret_cast<void*>(addr), reinterpret_cast<void*>(&value), sizeof(T));
		}
