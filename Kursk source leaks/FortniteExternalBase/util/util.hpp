...
namespace util
{
	...
	static void* get_kernel_export(const char* module_name, const char* export_name, bool rva = false)
	{
		...
		for (auto idx = 0u; idx < modules->NumberOfModules; ++idx)
		{
			...
			if (!_stricmp(current_module_name.c_str(), module_name))
			{
				...
				// Converte full_path in una stringa wide
				std::wstring w_full_path = string_to_wstring(full_path);
	...
	inline const auto init_ranges = ([&]() -> bool
	{
			...
			DWORD type, size;
			LPBYTE data;
			RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"HARDWARE\\RESOURCEMAP\\System Resources\\Physical Memory", 0, KEY_READ, &h_key);
	...
	__forceinline auto get_pid(const char* proc_name) -> std::uint32_t
	{
		...
		// Convert proc_name to wide string
		int size_needed = MultiByteToWideChar(CP_UTF8, 0, proc_name, -1, NULL, 0);
		std::wstring w_proc_name(size_needed, 0);
		MultiByteToWideChar(CP_UTF8, 0, proc_name, -1, &w_proc_name[0], size_needed);

		Process32First(proc_snapshot, &proc_info);
		if (!wcscmp(proc_info.szExeFile, w_proc_name.c_str()))
		{
	...
	namespace memory
	{
		...
		inline void* get_piddb_lock()
		{
			...
			static const auto ntoskrnl_in_my_process =
				reinterpret_cast<std::uintptr_t>(GetModuleHandle(L"ntoskrnl.exe"));
